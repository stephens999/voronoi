...
int NITER = 10000;
int BURNIN = 5000;
int VLENGTH = 1000; // length of vector of Voronoi points
int SAVANNAHONLY = 1;
...
void ReadInBoundary(ifstream & bfile, vector<double> & BoundaryX, vector<double> & BoundaryY)
{
   ...
   do{
      ...
      cerr << y << "," << x << endl;
	  if( BoundaryX.size() == 0 ) {
		  XMIN = XMAX = y;	//NB x and y swapped see comment above
		  YMIN = YMAX = x;
	  } else {
	      if( y < XMIN ) XMIN = y;
	      if( y > XMAX ) XMAX = y;
...
bool InRange(double x,double y){
  if( USEBOUNDARYFILE)
	  return (IsInsideBoundary( y, x, BoundaryX, BoundaryY ) != 0);
...
int main ( int argc, char** argv)
{
  ...
  while( ( argc > 1 ) && ( argv[1][0] == '-' ) ) {
   ...
   default: 
      cout << "Error: option " << argv[1] << "unrecognized" << endl;
      return 1;
  ...
  if(argc<3){
    cout << "Usage is ./VORONOI [-Bboundaryfile] [-G gridsize] [-V vlength] samplefile outfile " << endl;
    exit(1);
  }
  ...
  if( USEBOUNDARYFILE ) {
	  ...
	  //compute center and size of grid
	  double xcenter = (XMAX+XMIN)/2;
	  double ycenter = (YMAX+YMIN)/2;
	  double width = XMAX-XMIN;
	  double height = YMAX-YMIN;
	  if( height > width ) width = height;	//make it square
	  XMAX = xcenter + width/2;
	  XMIN = xcenter - width/2;
	  YMAX = ycenter + width/2;
	  YMIN = ycenter - width/2;

	  // write out grid boundary and size, and resulting cell size
	  string gridfilename = filenames["output"] + "_grid.txt";
	  ofstream gridfile( gridfilename.c_str());
	  gridfile << XMIN << " " << XMAX << " " << YMIN << " " << YMAX << " " << GRIDSIZE << " " << (width/GRIDSIZE) << endl;
	  gridfile.close();
  ...
  samplefile >> NIND;
  vector<string> samplefiles(NIND, "");
  for( int s = 0; s<NIND; s++) {
	  samplefile >> samplefiles[s];
  }
...
cout << "Reading " << TAG << endl;
     for(int s = 0; s < NIND; s++){
		 string LOCATEFILE;
		 LOCATEFILE = samplefiles[s];
		 cout << "file=" << LOCATEFILE <<endl;
	  ifstream locatefile (samplefilename.substr(0,samplefilename.find_last_of('\\')) + '\\' + LOCATEFILE + TAG);
      cout << (samplefilename.substr(0,samplefilename.find_last_of('\\')) + '\\' + LOCATEFILE + TAG);
	...
	if(!isinrange){
	  cout << "Warning: Not in range" << endl;
	  cout << "Sample =" << LOCATEFILE << endl;
	  cout << "Index = " << index << endl;
          cout << "x= " << x << endl;//<< PI*x/180 << endl;
	  cout << "y= " << y << endl;//PI*y/180 << endl;
	  cout << "dummy= " << dummy << endl;
	  //exit(1);
	}
	
	if(x>XMAX || x<XMIN || y>YMAX || y<YMIN){
	  cout << "Error: x or y out of bounds" << endl;
	  cout << "file = " << LOCATEFILE << endl;
	  cout << "x= " << x << endl;
	  cout << "y= " << y << endl;
	  exit(1);
	...
	int j = (int) (GRIDSIZE * (x-XMIN)/(XMAX-XMIN));
	int k = (int) (GRIDSIZE * (y-YMIN)/(YMAX-YMIN));
	
	//if(!GridInRange(j,k)){
	//	cout << "Error: grid square out of bounds" << endl;
	//exit(1);
	//}
...
//     cout << "A: " << ACCEPT << endl;
    ...
    if(iter>BURNIN){
 	...
 	for(int k=0; k<GRIDSIZE; k++){
	  ...
	  for(int i = 0; i<NIND; i++){
	    if(SUMCOUNTS[i]==0){
	      cout<< "Error: Sumcounts reached 0";
	      exit(1);
	    }
